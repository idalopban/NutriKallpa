...
// Hemoglobina - g/dL (valores varían por sexo)
function getDiagnosticoHemoglobina(value: number | undefined, sexo?: string): LabDiagnostic | null {
    if (!value || value <= 0) return null;

    const isMale = sexo === 'masculino';
    const normalMin = isMale ? 13.5 : 12.0;
    const normalMax = isMale ? 17.5 : 16.0;

    if (value < normalMin - 2) {
        return { severity: 'critico', label: 'Anemia Severa', message: `Valor muy bajo (<${normalMin - 2} g/dL). Evaluar causa urgente.`, suggestedPathology: 'Anemia' };
    } else if (value < normalMin) {
        return { severity: 'bajo', label: 'Anemia', message: `Valor bajo. Rango normal: ${normalMin}-${normalMax} g/dL.`, suggestedPathology: 'Anemia' };
    } else if (value >= normalMin && value <= normalMax) {
        return { severity: 'normal', label: 'Normal', message: `Hemoglobina dentro del rango normal.` };
    } else if (value > normalMax && value <= 20) {
        return { severity: 'precaucion', label: 'Elevada', message: 'Hemoglobina alta. Evaluar policitemia o deshidratación.' };
    } else {
        return { severity: 'critico', label: 'Muy Elevada', message: 'Valor críticamente alto. Evaluación urgente requerida.' };
    }
}
...
export function ClinicalHistoryTab({ patient }: ClinicalHistoryTabProps) {
    ...
    useEffect(() => {
        ...
        const diagnostics = [
            ...
            getDiagnosticoHemoglobina(biochem.hemoglobina, patientSex),
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    />
                                    <DiagnosticBadge diagnostic={getDiagnosticoHemoglobina(biochem.hemoglobina, patientSex)} />
                                </div>
