...
// Logic
import { getPacientes, getMedidasByPaciente, deletePaciente, getDietas } from "@/lib/storage";
...
function PatientRow({
  ...
  const dietas = getDietas(); // Ideally passed as prop for performance, but acceptable for now
...
function PacientesContent() {
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      // If no user ID, load all patients (fallback for demo/dev)
  ...
  const filteredPacientes = pacientes.filter((p) => {
    ...
    const medidas = getMedidasByPaciente(p.id);
    const dietas = getDietas();
    const status = getPatientStatus(medidas, dietas, p.id);

    if (filterStatus === "todos") return matchesSearch;
    if (filterStatus === "activos") return matchesSearch && status === "activo";
    if (filterStatus === "pendientes") return matchesSearch && status === "pendiente";
    if (filterStatus === "inactivos") return matchesSearch && status === "inactivo";
    return matchesSearch;
  });

  const allDietas = getDietas(); // Get all diets once for stats calculation
  const totalPacientes = pacientes.length;
  const pacientesActivos = pacientes.filter(p => getPatientStatus(getMedidasByPaciente(p.id), allDietas, p.id) === "activo").length;
  const pacientesPendientes = pacientes.filter(p => getPatientStatus(getMedidasByPaciente(p.id), allDietas, p.id) === "pendiente").length;
        ...
        <div className="flex flex-col gap-4">
          <div className="relative w-full">
            ...
            <Input
              ...
              className="pl-12 h-12 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-full text-base focus:ring-2 focus:ring-[#6cba00]/20 focus:border-[#6cba00] dark:focus:ring-green-500/20 dark:focus:border-green-500 shadow-sm"
              value={searchTerm}
