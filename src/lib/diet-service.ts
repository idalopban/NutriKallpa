...
export async function getUserDietHistoryAsync(userId: string): Promise<SavedPlan[]> {
    const supabasePlans = await supabaseStorage.getDietPlansFromSupabase(userId);
    ...
    if (supabasePlans.length > 0) {
        ...
        savePlansToStorage([...otherPlans, ...supabasePlans]);
...
export async function getPatientDietHistoryAsync(patientId: string): Promise<SavedPlan[]> {
    const supabasePlans = await supabaseStorage.getPatientDietPlansFromSupabase(patientId);
